...
Router = require config.path('model', 'router')
...
describe 'Film Router', ->
  describe 'Normalization', ->
    ...
    it 'normal', ->
      assert.deepEqual @router._normalize('/user'), ['/user', '']

    it 'with tail slash', ->
      assert.deepEqual @router._normalize('/user/'), ['/user', '']

    it 'with place holder', ->
      assert.deepEqual @router._normalize('/user/:id'), ['/user/:', 'id']

    it 'with name after place holder', ->
      assert.deepEqual @router._normalize('/user/:id/address'), ['/user/:/address', 'id']

    it 'with name and tail slash after place holder', ->
      assert.deepEqual @router._normalize('/user/:id/address/'), ['/user/:/address', 'id']

    it 'with multiple place holder', ->
      assert.deepEqual @router._normalize('/user/:id/address/:address_id'), ['/user/:/address/:', 'id:address_id']
